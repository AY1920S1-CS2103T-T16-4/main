...
.. Test case: `edit 1` +
   Expected: No meme is edited. Weme prompts the user to specify at least one field to be edited.
.. Other incorrect add commands to try: `edit`, +
   Expected: Similar to previous.
...
. Deletes a meme while all memes are listed
...
.. Test case: `delete 0` +
   Expected: No meme is deleted. Weme prompts the user to give a valid index.
.. Other incorrect add commands to try: `delete`, `delete x` (where x is larger than the list size) +
   Expected: Similar to previous.
...
. Clears all memes in the memes tab
