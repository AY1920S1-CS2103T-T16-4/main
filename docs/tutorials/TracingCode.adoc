...
public CommandResult execute(Model model) throws CommandException {
    ...
    Person editedMeme = createEditedPerson(memeToEdit, editMemeDescriptor);
    if (!memeToEdit.isSamePerson(editedMeme) && model.hasPerson(editedMeme)) {
        throw new CommandException(MESSAGE_DUPLICATE_PERSON);
    }
    model.setPerson(memeToEdit, editedMeme);
    model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);
    return new CommandResult(String.format(MESSAGE_EDIT_PERSON_SUCCESS, editedMeme));
}
----
