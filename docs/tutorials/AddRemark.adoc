...
/**
 * Changes the remark of an existing meme in the address book.
 */
...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the meme identified "
            + "by the index number used in the last meme listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the meme in the filtered meme list to edit the remark
     * @param remark of the meme to be updated to
     */
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the meme's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a meme.
...
Without getting too deep into `fxml`, let's go on a 5 minute adventure to get some placeholder text to show up for each meme.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.weme.ui.MemeCard`].
...
.PersonCard.java
----
public PersonCard(Person meme, int displayedIndex) {
    //...
    remark.setText(meme.getRemark().value);
}
----
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person memeToEdit = lastShownList.get(index.getZeroBased());
        Person editedMeme = new Person(memeToEdit.getName(), memeToEdit.getPhone(), memeToEdit.getEmail(),
                memeToEdit.getAddress(), remark, memeToEdit.getTags());

        model.setPerson(memeToEdit, editedMeme);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedMeme));
    }
     ...
     * {@code memeToEdit}.
     */
    private String generateSuccessMessage(Person memeToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, memeToEdit);
    }
----
